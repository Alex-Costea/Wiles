...
class DataStatement(context: ParserContext) : AbstractStatement(context) {
    ...
    override fun process(): CompilationExceptionsCollection {
        ...
        try{
            transmitter.expect(ExpectParamsBuilder.tokenOf(Tokens.BRACE_START_ID))
            while(transmitter.expectMaybe(ExpectParamsBuilder.tokenOf(Tokens.BRACE_END_ID).removeWhen(WhenRemoveToken.Never)).isEmpty)
            {
            ...
            location = transmitter.expect(ExpectParamsBuilder.tokenOf(Tokens.BRACE_END_ID)).location
        }
