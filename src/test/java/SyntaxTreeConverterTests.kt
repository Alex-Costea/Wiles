...
class SyntaxTreeConverterTests {
    ...
    @Test
    fun expressionsTest() {
        assertResults(null, "CODE_BLOCK(DECLARATION(EXPRESSION(!main); METHOD(TYPE NOTHING; CODE_BLOCK(EXPRESSION(EXPRESSION(!b); ASSIGN; EXPRESSION(!c))))))",
                DECLARE_ID, "!main", ASSIGN_ID, METHOD_ID,
                ROUND_BRACKET_START_ID, ROUND_BRACKET_END_ID, START_BLOCK_ID,
                "!b", ASSIGN_ID, "!c", END_BLOCK_ID)
        assertResults(null, "CODE_BLOCK(DECLARATION(EXPRESSION(!main); METHOD(TYPE NOTHING; CODE_BLOCK(EXPRESSION(EXPRESSION(!b); ASSIGN; EXPRESSION(#3))))))",
            DECLARE_ID, "!main", ASSIGN_ID, METHOD_ID,
            ROUND_BRACKET_START_ID, ROUND_BRACKET_END_ID, START_BLOCK_ID,
    ...
    fun methodTest() {
        ...
        assertResults(null, "CODE_BLOCK(DECLARATION(EXPRESSION(!main); METHOD(TYPE INT32; PARAMETER(!a; TYPE NOTHING); CODE_BLOCK)))",
            ...
            RIGHT_ARROW_ID, "!int", START_BLOCK_ID, END_BLOCK_ID)
        assertResults(null, "CODE_BLOCK(DECLARATION(EXPRESSION(!a); METHOD(TYPE NOTHING; CODE_BLOCK(EXPRESSION(NOTHING)))))",
        ...
        assertResults(null,"CODE_BLOCK(DECLARATION(EXPRESSION(!main); METHOD(TYPE NOTHING; PARAMETER ANON(!arg1; TYPE INT32); CODE_BLOCK(EXPRESSION(NOTHING)))))",
            DECLARE_ID, "!main", ASSIGN_ID, METHOD_ID, ROUND_BRACKET_START_ID, "!arg1", TYPEOF_ID, "!int", ROUND_BRACKET_END_ID, DO_ID, NOTHING_ID)
        assertResults(null,"CODE_BLOCK(DECLARATION(EXPRESSION(!a); METHOD(TYPE NOTHING; CODE_BLOCK(DECLARATION(EXPRESSION(!b); METHOD(TYPE NOTHING; CODE_BLOCK(EXPRESSION(NOTHING))))))))",
