...
class SyntaxTreeConverterTests {
    ...
    fun methodTest() {
        ...
        assertResults(null, "CODE_BLOCK(DECLARATION(!main; EXPRESSION(METHOD(TYPE INT32; PARAMETER(!a; TYPE INT32); CODE_BLOCK))))",
        ...
        assertResults(null, "CODE_BLOCK(DECLARATION(!main; EXPRESSION(METHOD(TYPE INT32; PARAMETER(!a; TYPE INT32); PARAMETER(!b; TYPE STRING); CODE_BLOCK))))",
        ...
        assertResults(null, "CODE_BLOCK(DECLARATION(!main; EXPRESSION(METHOD(TYPE INT32; PARAMETER(!a; TYPE NOTHING); CODE_BLOCK))))",
        ...
        assertResults(null, "CODE_BLOCK(DECLARATION(!main; EXPRESSION(METHOD(TYPE NOTHING; CODE_BLOCK(EXPRESSION(EXPRESSION(!b); ASSIGN; EXPRESSION(#3)))))))",
            DECLARE_ID, "!main", ASSIGN_ID, METHOD_ID, PAREN_START_ID, PAREN_END_ID, DO_ID,
             "!b", ASSIGN_ID, "#3")
        assertResults(null, "CODE_BLOCK(DECLARATION(!product; EXPRESSION(METHOD(TYPE INT64; PARAMETER(!a; TYPE INT32); PARAMETER(!b; TYPE INT32); CODE_BLOCK(EXPRESSION(EXPRESSION(!product); ASSIGN; EXPRESSION(!a; TIMES; !b)))))))",
        ...
        assertResults(null,"CODE_BLOCK(DECLARATION(!main; EXPRESSION(METHOD(TYPE NOTHING; PARAMETER(!args; TYPE INT32); CODE_BLOCK(EXPRESSION(NOTHING))))))",
